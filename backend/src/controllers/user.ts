...
class UsersController {
  static getAll: RequestHandler = async (req, res, next) => {
    try {
      ...
      if (!users.length)
        return res
          .status(404)
          .json(new ResJSON([], false, "Could not found any users"));
  ...
  static getByEmail: RequestHandler = async (req, res, next) => {
    if (!req.body?.email)
      return res
        .status(400)
        .json(new ResJSON([], false, "Email was not provided"));
  ...
  static create: RequestHandler = async (req, res, next) => {
    ...
    if (!body.name || !body.email || !body.password)
      return res.status(400).json(new ResJSON([], false, "Missing fields"));

    const saltRounds = 10;
    const passwordHash: string = await bcrypt.hash(body.password, saltRounds);
    if (!passwordHash)
      return res.status(500).json(new ResJSON([], false, "Server error"));
    ...
    if (!userValidation.success)
      return res
        .status(400)
        .json(new ResJSON([], false, "One or more fields are not valid"));
    ...
    try {
      ...
      if (!user)
        return res
          .status(400)
          .json(new ResJSON([], false, "Email is already in use"));

      return res
        .status(201)
        .json(new ResJSON([user], true, "User created successfully"));
    } catch (error) {
      next(error);
    }
  };
  ...
  static delete: RequestHandler = async (req, res, next) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json(new ResJSON([user], true, "User successfully deleted"));
    } catch (error) {
      next(error);
    }
  };
  ...
  static update: RequestHandler = async (req, res, next) => {
    ...
    if (userFromToken?.id !== id)
      return res
        .status(403)
        .json(new ResJSON([], false, "Cannot update this user"));
    if (typeof id !== "string") return res.status(400).json(InvalidIDJSON);
    if (!body)
      return res.status(400).json(new ResJSON([], false, "No data was send"));
    ...
    if (body.email) {
      ...
      if (user)
        return res
          .status(400)
          .json(new ResJSON([], false, "Email already in use"));
    }
    ...
    try {
      ...
      if (!user)
        return res
          .status(404)
          .json(new ResJSON([], false, "Could not update the user"));

      return res
        .status(200)
        .json(new ResJSON([user], true, "User successfully updated"));
    } catch (error) {
      next(error);
    }
  };
  ...
  static login: RequestHandler = async (req, res, next) => {
      ...
      return res
        ...
        .json(
          new ResJSON(
            [],
            false,
            "Missing fields, must provide email and password",
          ),
        );
    ...
    if (!userValidation.success)
      return res
        .status(400)
        .json(new ResJSON([], false, "One or more fields have a wrong format"));

    try {
      const [user] = await UserModel.getByEmail({ email });

      const isCorrectPassword: boolean = !user
        ? false
      ...
      if (!user || !isCorrectPassword)
        return res
          .status(401)
          .json(new ResJSON([], false, "Invalid credentials"));
      ...
      return res
        .status(200)
        .json(new ResJSON([userForRes], true, "Access granted"));
    } catch (error) {
      next(error);
    }
  };
